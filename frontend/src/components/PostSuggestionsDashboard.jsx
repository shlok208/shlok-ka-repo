...
const PostSuggestionsDashboard = () => {
  ...
  const fetchSuggestedPosts = async () => {
    try {
      ...
      const result = await contentAPI.getCreatedContent(50, 0)
  ...
  const handleDeletePost = async (post) => {
    ...
    try {
      ...
      const response = await fetch(`${API_BASE_URL}/content/post-contents/${post.id}`, {
        method: 'DELETE',
        headers: {
      ...
      setSuggestedPosts(prev => prev.filter(p => p.id !== post.id))
